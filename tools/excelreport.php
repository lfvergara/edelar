...
class ExcelReport extends View {
  ...
  function extraer_informe($titulo,$array) {
    date_default_timezone_set('America/Mexico_City');
    if (PHP_SAPI == 'cli') die('Este archivo solo se puede ver desde un navegador web');

    $objPHPExcel = new PHPExcel();
    $objPHPExcel->getProperties()->setCreator("EDELAR S.A.") //Autor
                                 ->setLastModifiedBy("EDELAR S.A.") //Ultimo usuario que lo modificó
                                 ->setTitle("Exportador CV")
                                 ->setSubject("Exportador CV")
                                 ->setDescription("Exportador CV")
                                 ->setKeywords("Exportador CV")
                                 ->setCategory("Exportador CV");

    $tituloReporte = "Exportador CV";
    ...
    $objPHPExcel->getActiveSheet()->setTitle("Exportador CV");
    $objPHPExcel->setActiveSheetIndex(0);
