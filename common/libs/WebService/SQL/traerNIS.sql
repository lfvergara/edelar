SELECT    NN.ID_NOTICE AS id_deuda,
    ...
    FROM   GCCB_NIR_NOTICE NN,
           ...
           GCCD_RELATIONSHIP RS,
           GCCC_CUSTOMER_TYPE CTYPE,
           GCCB_NOTICE_COMPOSITION NC,
           GCCB_XML_NOTICE NXML
   WHERE   N.ID_NOTICE = NN.ID_NOTICE
           AND N.ID_NOTICE = NC.ID_NOTICE
           AND NXML.ID_XML_NOTICE = NC.ID_XML_NOTICE
           ...
           AND RS.ID_RELATIONSHIP = N.ID_CUSTOMER
           AND RS.CUSTOMER_TYPE = CTYPE.COD_DEVELOP
           AND N.ID_NOTICE = NC.ID_NOTICE (+)
           AND (NC.ID_STATEMENT_LOT IS NULL
           OR (NC.ID_STATEMENT_LOT IN
             (SELECT ID_STATEMENT_LOT FROM GCPR_PRINTING_LOT WHERE
      PRINTING_LOT_STATUS = 'PRLOTST004' )))
           AND CTYPE.COD_DEVELOP NOT IN ('TYPCUS0105',
                     'TYPCUS0106',
                     'TYPCUS0107',
                     'TYPCUS0108',
                     'TYPCUS0109',
